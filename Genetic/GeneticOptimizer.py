...
class GeneticOptimizer:
    def __init__(self, model_class, initI, initN, params, param_ranges, error_func, real_data, mut_range=0.05, p_mut=0.2, p_ind=0.5, p_regen=0.1, p_cross=0.5, period=15, stop_cond=-1,
        ...
        self.toolbox.register("individual", self.gen_individual, self, creator.FitnessMax())
        self.toolbox.register("population", tools.initRepeat, list, self.toolbox.individual)

        self.toolbox.register("evaluate", self.fit_func, self=self)
        self.toolbox.register("mate", self.cross_func, self=self)
        self.toolbox.register("mutate", self.mut_func, self=self)
        self.toolbox.register("select", tools.selTournament, tournsize=5)

    def gen_individual(self, fitness):
        return self.model_class(initI=self.initI, initN=self.initN, fitness=fitness, **self.params)

    def fit_func(self, model):
        model.run(T=self.period)
        predicted_values = model.total_num_infections()[10::10]
        ...
        for param in self.params:
            if rnd.rand() < self.p_ind:
                if rnd.rand() < self.p_regen:
                    setattr(model, param, rnd.rand() * (self.param_ranges[param][1] - self.param_ranges[param][0]) +
                            self.param_ranges[param][0])
                else:
                    setattr(model, param, getattr(model, param) + rnd.rand() * self.mut_range * 2 - self.mut_range)

    def cross_func(self, model1, model2):
        for param in self.params:
            if rnd.rand() < self.p_ind:
                val1 = getattr(model1, param)
                setattr(model1, param, getattr(model2, param))
                setattr(model2, param, val1)
    ...
    def iteration(self, real_data):
        # Begin the evolution
        if max(self.fits) < self.stop_cond:
        ...
        print("  Avg %s" % mean)
        print("  Std %s" % std)
        best = self.pop[np.argmax(self.fits)]
